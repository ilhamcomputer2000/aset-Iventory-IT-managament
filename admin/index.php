...
// Session variables (untuk sidebar)
...
// (Sering terjadi di hosting: session tidak selalu membawa semua kolom.)
if ($Jabatan_Level === '' && isset($_SESSION['user_id'])) {
    ...
    if ($stmtMeta) {
        ...
        error_log('Failed to prepare user meta query (admin/index.php): ' . $kon->error);
    }
}
...
function admin_update_url($idPeserta)
{
    $id = (int) $idPeserta;
    if ($id <= 0) {
        return 'update.php';
    ...
    if (is_string($t) && $t !== '') {
        return 'update.php?t=' . rawurlencode($t);
    }
    // Fallback jika hosting tidak mendukung OpenSSL/random_bytes
    return 'update.php?id_peserta=' . rawurlencode((string) $id);
}

// TAMBAHAN: Logika Delete Data (handle ?delete=true&id_peserta=ID)
if (isset($_GET['delete']) && isset($_GET['id_peserta']) && $_GET['delete'] === 'true') {
    $id_peserta = intval($_GET['id_peserta']);  // Escape dan cast ke int untuk security (hindari SQL injection)

    if ($id_peserta > 0) {
        // Cek apakah ID exist di DB (prevent delete non-exist)
        $check_query = "SELECT id_peserta, Photo_Barang FROM peserta WHERE id_peserta = $id_peserta";
        $check_result = mysqli_query($kon, $check_query);

        if (mysqli_num_rows($check_result) > 0) {
            $row = mysqli_fetch_assoc($check_result);
            $photo_path = $row['Photo_Barang'];

            // Hapus file foto jika ada (cleanup storage)
            if (!empty($photo_path) && file_exists("../uploads/" . $photo_path)) {
                unlink("../uploads/" . $photo_path);  // Hapus file fisik
            }

            // Hapus data dari DB
            $delete_query = "DELETE FROM peserta WHERE id_peserta = $id_peserta";
            $delete_result = mysqli_query($kon, $delete_query);

            if ($delete_result) {
                // Sukses: Redirect dengan success=1 untuk alert
                header("Location: index.php?success=1");
                exit();
            } else {
                // Gagal: Redirect dengan error=1
                header("Location: index.php?error=1&message=" . urlencode("Gagal menghapus data: " . mysqli_error($kon)));
                exit();
            }
        } else {
            // ID tidak exist
            header("Location: index.php?error=1&message=" . urlencode("Data tidak ditemukan."));
            exit();
        }
    } else {
        // ID invalid
        header("Location: index.php?error=1&message=" . urlencode("ID tidak valid."));
        exit();
    }
}

// TAMBAHAN: Handle Success/Error Message dari Redirect (tampilkan alert jika ada parameter)
$success_message = '';
$error_message = '';
...
if (isset($_GET['action']) && $_GET['action'] === 'ajax_get_assets') {
    ...
    if ($ajax_view_mode === 'table') {
        ...
        while ($ajax_data = mysqli_fetch_array($ajax_result)) {
            ...
            $ajax_table_html .= '<tr class="table-row border-b border-gray-100 ' . $ajax_rowClass . ' hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50">
                                ...
                                <td class="px-4 py-4 text-gray-700 sticky-col col-createby">' . htmlspecialchars($_SESSION['username'] ?? '-') . ' - ' . htmlspecialchars($_SESSION['Nama_Lengkap'] ?? 'Unknown') . '</td>
                                ...
                                <td class="px-4 py-4 sticky-right">
                                    <div class="flex items-center justify-center space-x-2">
                                        <a href="' . htmlspecialchars(admin_update_url($ajax_data['id_peserta'] ?? 0), ENT_QUOTES) . '" class="p-2 text-green-600"><i class="fas fa-edit"></i></a>
                                        <button type="button" class="transfer-btn p-2 text-indigo-600 transition-transform hover:rotate-180" data-id="' . htmlspecialchars($ajax_data['id_peserta'] ?? '') . '" title="Transfer Aset IT"><i class="fas fa-right-left"></i></button>
                                        <button class="delete-btn p-2 text-red-600" data-id="' . htmlspecialchars($ajax_data['id_peserta'] ?? '') . '"><i class="fas fa-trash"></i></button>
        ...
        while ($ajax_data = mysqli_fetch_array($ajax_result)) {
            ...
            $gridPhotos = [];
            if (!empty($ajax_data['Photo_Barang']))
                $gridPhotos[] = '../uploads/' . $ajax_data['Photo_Barang'];
            if (!empty($ajax_data['Photo_Depan']))
                $gridPhotos[] = '../uploads/' . $ajax_data['Photo_Depan'];
            if (!empty($ajax_data['Photo_Belakang']))
                $gridPhotos[] = '../uploads/' . $ajax_data['Photo_Belakang'];
            if (!empty($ajax_data['Photo_SN']))
                $gridPhotos[] = '../uploads/' . $ajax_data['Photo_SN'];
                ...
                . '<div class="absolute top-1 left-1">
                                    ...
                                    <div class="p-2 flex flex-col flex-grow">
                                        ...
                                        <div class="flex gap-0.5 pt-1 border-t border-gray-100 mt-auto">
                                            <a href="' . htmlspecialchars(admin_update_url($idPeserta), ENT_QUOTES) . '" class="flex-1 p-0.5 text-green-600 hover:bg-green-50 rounded border border-green-200 hover:border-green-300 transition-all text-center" title="Edit">
                                                <i class="fas fa-edit text-[9px]"></i>
                                            </a>
                                            <button type="button" class="transfer-btn flex-1 p-0.5 text-indigo-600 hover:bg-indigo-50 rounded border border-indigo-200 hover:border-indigo-300 transition-all text-center hover:rotate-180" data-id="' . htmlspecialchars($idPeserta) . '" title="Transfer Aset IT">
                                                <i class="fas fa-right-left text-[9px]"></i>
                                            </button>
                                            <button class="delete-btn flex-1 p-0.5 text-red-600 hover:bg-red-50 rounded border border-red-200 hover:border-red-300 transition-all text-center" data-id="' . htmlspecialchars($idPeserta) . '" title="Delete">
    ...
    if ($ajax_total_pages > 1) {
        ...
        $ajax_base_params = [];
        if (!empty($ajax_status_filter))
            $ajax_base_params['status_filter'] = $ajax_status_filter;
        if (!empty($ajax_category_filter))
            $ajax_base_params['kategori'] = $ajax_category_filter;
        if (!empty($ajax_search_query))
            $ajax_base_params['search'] = $ajax_search_query;
        if (!empty($ajax_brand_filter))
            $ajax_base_params['brand_filter'] = $ajax_brand_filter;
        if (!empty($ajax_status_lop_filter))
            $ajax_base_params['status_lop_filter'] = $ajax_status_lop_filter;
        if (!empty($ajax_status_kelayakan_filter))
            $ajax_base_params['status_kelayakan_filter'] = $ajax_status_kelayakan_filter;
        if (!empty($ajax_start_date))
            $ajax_base_params['start_date'] = $ajax_start_date;
        if (!empty($ajax_end_date))
            $ajax_base_params['end_date'] = $ajax_end_date;
        if (!empty($ajax_view_mode))
            $ajax_base_params['view_mode'] = $ajax_view_mode;
...
if ($start_date !== '')
    $download_excel_params['start_date'] = $start_date;
if ($end_date !== '')
    $download_excel_params['end_date'] = $end_date;
...
// ===== Transfer Aset IT: opsi Select2 (search + bisa tambah) =====
// Ambil opsi unik (dibatasi agar tidak overload)
$transfer_user_perangkat_options = [];
$transfer_jabatan_options = [];
$transfer_employee_id_options = [];
$transfer_lokasi_options = [];

$q1 = "SELECT DISTINCT Nama_Lengkap AS v FROM users WHERE Nama_Lengkap IS NOT NULL AND TRIM(Nama_Lengkap) != '' ORDER BY Nama_Lengkap ASC LIMIT 500";
$r1 = mysqli_query($kon, $q1);
if ($r1) {
    while ($row = mysqli_fetch_assoc($r1)) {
        $val = trim((string) ($row['v'] ?? ''));
        if ($val !== '')
            $transfer_user_perangkat_options[] = $val;
    }
}

$q2 = "SELECT DISTINCT Jabatan_Level AS v FROM users WHERE Jabatan_Level IS NOT NULL AND TRIM(Jabatan_Level) != '' ORDER BY Jabatan_Level ASC LIMIT 200";
$r2 = mysqli_query($kon, $q2);
if ($r2) {
    while ($row = mysqli_fetch_assoc($r2)) {
        $val = trim((string) ($row['v'] ?? ''));
        if ($val !== '')
            $transfer_jabatan_options[] = $val;
    }
}

$q3 = "SELECT DISTINCT username AS v FROM users WHERE username IS NOT NULL AND TRIM(username) != '' ORDER BY username ASC LIMIT 500";
$r3 = mysqli_query($kon, $q3);
if ($r3) {
    while ($row = mysqli_fetch_assoc($r3)) {
        $val = trim((string) ($row['v'] ?? ''));
        if ($val !== '')
            $transfer_employee_id_options[] = $val;
    }
}

$q4 = "SELECT DISTINCT Region AS v FROM users WHERE Region IS NOT NULL AND TRIM(Region) != '' ORDER BY Region ASC LIMIT 200";
$r4 = mysqli_query($kon, $q4);
if ($r4) {
    while ($row = mysqli_fetch_assoc($r4)) {
        $val = trim((string) ($row['v'] ?? ''));
        if ($val !== '')
            $transfer_lokasi_options[] = $val;
    }
}

$transfer_user_perangkat_options = array_values(array_unique($transfer_user_perangkat_options));
$transfer_jabatan_options = array_values(array_unique($transfer_jabatan_options));
$transfer_employee_id_options = array_values(array_unique($transfer_employee_id_options));
$transfer_lokasi_options = array_values(array_unique($transfer_lokasi_options));
...
function getStatusBadgeClass($status)
{
    switch (strtoupper(trim($status))) {
        ...
        case 'IN USE':
            return 'bg-red-600 text-white border-red-700'; // ✅ Red background & white text
        case 'KOSONG':
...
<head>
    ...
    <style>
        /* Transfer modal: Select2 helpers (search + taggable) */
        #transferPanel .transfer-select+.select2-container {
            width: 100% !important;
        }

        #transferPanel .transfer-select+.select2-container .select2-selection {
        ...
        #transferPanel .transfer-select+.select2-container .select2-selection__rendered {
            line-height: 38px;
        }

        #transferPanel .transfer-select+.select2-container .select2-selection__arrow {
            height: 38px;
        }

        #transferModal .select2-dropdown {
            z-index: 99999;
        }

        #transferPanel .transfer-field-icon {
            z-index: 2;
            pointer-events: none;
        }
        ...
        .table-sticky .col-waktu {
            left: 50px;
            min-width: 110px;
        }

        .table-sticky .col-createby {
            left: 160px;
            min-width: 150px;
        }

        .table-sticky .col-namabarang {
            left: 310px;
            min-width: 180px;
        }

        .table-sticky .col-nomoraset {
            min-width: 140px;
        }
...
<body
    ...
    <div id="main-content-wrapper" class="main-content lg:ml-60 min-h-screen transition-all duration-300 ease-in-out">
        ...
        <!-- Search Bar (moved from old navbar) -->
        <div class="bg-white shadow-sm border-b border-gray-200 px-6 py-3 mt-16">
            <div class="flex flex-col sm:flex-row items-center gap-3">
                <form action="index.php" method="get" class="relative flex-1 w-full">
                    <div class="flex items-center">
                        <input type="search" name="search" placeholder="Search assets..."
                            value="<?php echo htmlspecialchars($search_query); ?>"
                            class="w-full px-4 py-2 pr-12 rounded-lg border border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white text-gray-800 placeholder:text-gray-500"
                        ...
                        <input type="hidden" name="brand_filter" value="<?php echo htmlspecialchars($brand_filter); ?>">
                        <input type="hidden" name="status_lop_filter"
                            value="<?php echo htmlspecialchars($status_lop_filter); ?>">
                        <input type="hidden" name="start_date" value="<?php echo htmlspecialchars($start_date); ?>">
                        <input type="hidden" name="end_date" value="<?php echo htmlspecialchars($end_date); ?>">
                        <input type="hidden" name="view_mode" value="<?php echo htmlspecialchars($view_mode); ?>">
                        <button type="submit"
                            class="absolute right-2 p-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-all duration-300">
        ...
        <div class="p-6">
            ...
            <div class="mb-8 text-center">
                <h1
                    class="text-4xl font-bold bg-gradient-to-r from-orange-600 to-orange-800 bg-clip-text text-transparent mb-3">
                    DAFTAR ASSET IT
                </h1>
            ...
            <div class="flex flex-col sm:flex-row gap-4 mb-6">
                <a href="create.php"
                    class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-medium rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                    <i class="fas fa-plus mr-2"></i>
                    Tambah Data
                </a>
                <a href="<?php echo htmlspecialchars($download_excel_href, ENT_QUOTES); ?>"
                    class="inline-flex items-center px-6 py-3 border-2 border-green-500 text-green-600 font-medium rounded-lg shadow-md hover:shadow-lg hover:bg-green-50 transition-all duration-300 hover:scale-105">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-xl p-4 mb-6 shadow-lg border border-gray-200">
                <form method="GET" action="index.php" class="flex flex-col lg:flex-row gap-4">
                    <!-- Filter Section -->
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- 3 kolom desktop, responsive -->
                            <!-- Kategori (div 1) -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kategori:</label>
                                <select id="kategori-filter" name="kategori"
                                    class="select2-filter w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm max-h-40 overflow-y-auto"
                            ...
                            <div>
                                ...
                                <select name="status_filter"
                                    class="select2-filter w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm"
                                    data-placeholder="Pilih atau ketik status...">
                                    <option value="">Semua Status</option>
                                    <option value="READY" <?php echo ($status_filter == 'READY') ? 'selected' : ''; ?>>
                                        Ready</option>
                                    <option value="KOSONG" <?php echo ($status_filter == 'KOSONG') ? 'selected' : ''; ?>>
                                        Kosong</option>
                                    <option value="IN USE" <?php echo ($status_filter == 'IN USE') ? 'selected' : ''; ?>>
                                        IN USE</option>
                                    ...
                                    <option value="RUSAK" <?php echo ($status_filter == 'RUSAK') ? 'selected' : ''; ?>>
                                        Rusak</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="brand-filter" name="brand_filter"
                                    class="select2-filter w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm"
                                    data-placeholder="Pilih atau ketik merek...">
                            ...
                            <div>
                                ...
                                <select name="status_lop_filter"
                                    class="select2-filter w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm"
                                    data-placeholder="Pilih atau ketik Status LOP...">
                            ...
                            <div>
                                ...
                                <select name="status_kelayakan_filter"
                                    class="select2-filter w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm"
                                    data-placeholder="Pilih atau ketik Status Kelayakan...">
                            ...
                            <div>
                                ...
                                <input type="date" name="start_date"
                                    ...
                                    class="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/90 backdrop-blur-sm text-sm" />
                            </div>
                    ...
                    <div class="flex flex-col items-center lg:items-end justify-center gap-4">
                        <div class="flex flex-col items-center">
                            ...
                            <div class="flex bg-gray-100 rounded-lg p-1">
                                <button type="button" onclick="changeViewMode('table')"
                                    class="flex items-center space-x-2 px-4 py-2 rounded-md transition-all duration-200 <?php echo ($view_mode === 'table') ? 'bg-white text-blue-600 shadow-md' : 'text-gray-600 hover:text-gray-800'; ?>">
                                    <i class="fas fa-list"></i>
                                    <span class="hidden sm:inline">Table</span>
                                </button>
                                <button type="button" onclick="changeViewMode('grid')"
                                    class="flex items-center space-x-2 px-4 py-2 rounded-md transition-all duration-200 <?php echo ($view_mode === 'grid') ? 'bg-white text-blue-600 shadow-md' : 'text-gray-600 hover:text-gray-800'; ?>">
                                    <i class="fas fa-th"></i>
                        ...
                        <button type="submit"
                            class="px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105">
            ...
            <?php if ($view_mode === 'grid'): ?>
                ...
                <div
                    class="asset-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 mb-8">
                    <?php
                    $no = $start_from + 1;
                    while ($data = mysqli_fetch_array($hasil)) {
                        $grid_photos = [];
                        if (!empty($data['Photo_Barang']))
                            $grid_photos[] = "../uploads/" . $data['Photo_Barang'];
                        if (!empty($data['Photo_Depan']))
                            $grid_photos[] = "../uploads/" . $data['Photo_Depan'];
                        if (!empty($data['Photo_Belakang']))
                            $grid_photos[] = "../uploads/" . $data['Photo_Belakang'];
                        if (!empty($data['Photo_SN']))
                            $grid_photos[] = "../uploads/" . $data['Photo_SN'];
                        ...
                        ?>
                        <div class="bg-white/90 backdrop-blur-sm rounded-lg shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-200 overflow-hidden group flex flex-col h-full"
                            data-photos="<?php echo $grid_photos_json; ?>" data-photo-index="0">
                            <!-- Image Section -->
                            <div class="relative h-32 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                                <?php if (!empty($grid_primary_photo)): ?>
                                    <img src="<?php echo htmlspecialchars($grid_primary_photo); ?>"
                                ...
                                <div class="absolute top-1 left-1">
                                    <span
                                        class="inline-block px-1.5 py-0.5 rounded-full text-xs font-semibold border <?php echo getStatusBadgeClass($data['Status_Barang']); ?>">
                            ...
                            <div class="p-2 flex flex-col flex-grow">
                                <!-- Title -->
                                <h3 class="font-semibold text-[10px] text-gray-900 line-clamp-2 mb-0.5"
                                    title="<?php echo htmlspecialchars($data['Nama_Barang']); ?>">
                                    <?php echo htmlspecialchars($data['Nama_Barang']); ?>
                                </h3>

                                <!-- Brand -->
                                <p class="text-[9px] text-gray-600 mb-0.5 truncate">
                                    <?php echo htmlspecialchars($data['Merek']); ?></p>

                                <!-- Type Badge -->
                                <div class="mb-1">
                                    <span
                                        class="inline-block px-1 py-0.5 rounded text-[8px] font-semibold bg-blue-100 text-blue-700">
                                ...
                                <div class="text-[8px] text-gray-600 space-y-0.5 mb-1">
                                    <div class="truncate">
                                        <i class="fas fa-user text-gray-400 w-3"></i>
                                        <span
                                            class="truncate inline-block max-w-[75%]"><?php echo htmlspecialchars(!empty($data['User_Perangkat']) ? $data['User_Perangkat'] : '-'); ?></span>
                                    </div>
                                    <div class="truncate">
                                        <i class="fas fa-briefcase text-gray-400 w-3"></i>
                                        <span
                                            class="truncate inline-block max-w-[75%]"><?php echo htmlspecialchars(!empty($data['Jabatan']) ? $data['Jabatan'] : '-'); ?></span>
                                    </div>
                                    <div class="truncate">
                                        <i class="fas fa-id-card text-gray-400 w-3"></i>
                                        <span
                                            class="truncate inline-block max-w-[75%]"><?php echo htmlspecialchars(!empty($data['Id_Karyawan']) ? $data['Id_Karyawan'] : '-'); ?></span>
                                    </div>
                                    <div>
                                        <div class="flex items-start gap-1">
                                            <i class="fas fa-clipboard-list text-gray-400 w-3 mt-[1px]"></i>
                                            <div class="min-w-0 flex-1">
                                                <div class="flex items-start justify-between gap-1">
                                                    <span class="truncate inline-block max-w-[75%]"
                                                        title="<?php echo htmlspecialchars($data['Spesifikasi'] ?? ''); ?>">
                                                        <?php echo htmlspecialchars(!empty($data['Spesifikasi']) ? $data['Spesifikasi'] : '-'); ?>
                                                    </span>
                                                    <?php if (!empty($data['Spesifikasi'])): ?>
                                                        <button type="button"
                                                            class="grid-spec-toggle text-gray-500 hover:text-gray-700 px-1"
                                                            aria-expanded="false" aria-label="Lihat detail spesifikasi">
                                                            <i
                                                                class="fas fa-chevron-down grid-spec-chevron text-[9px] transition-transform duration-200"></i>
                                                        </button>
                                                    <?php endif; ?>
                                                </div>
                                                <?php if (!empty($data['Spesifikasi'])): ?>
                                    ...
                                    <div class="truncate">
                                        <i class="fas fa-map-marker-alt text-gray-400 w-3"></i>
                                        <span
                                            class="truncate inline-block max-w-[75%]"><?php echo htmlspecialchars($data['Lokasi']); ?></span>
                                    </div>
                                    <div class="truncate">
                                        <i class="fas fa-calendar text-gray-400 w-3"></i>
                                ...
                                <div class="mb-1">
                                    <span
                                        class="text-[7px] font-mono bg-gray-100 px-1 py-0.5 rounded text-gray-700 block truncate">
                                ...
                                <div class="flex flex-wrap gap-0.5 mb-1">
                                    <span
                                        class="inline-block px-1 py-0.5 rounded text-[7px] font-medium border <?php echo getStatusBadgeClass($data['Jenis_Barang']); ?>">
                                        <?php echo htmlspecialchars(substr($data['Jenis_Barang'], 0, 5)); ?>
                                    </span>
                                    <span
                                        class="inline-block px-1 py-0.5 rounded text-[7px] font-medium border <?php echo getStatusBadgeClass($data['Status_Kelayakan_Barang']); ?>">
                                ...
                                <div class="flex gap-0.5 pt-1 border-t border-gray-100 mt-auto">
                                    <a href="<?php echo htmlspecialchars(admin_update_url($data['id_peserta']), ENT_QUOTES); ?>"
                                        class="flex-1 p-0.5 text-green-600 hover:bg-green-50 rounded border border-green-200 hover:border-green-300 transition-all text-center"
                                        title="Edit">
                                        <i class="fas fa-edit text-[9px]"></i>
                                    </a>
                                    <button type="button"
                                        class="transfer-btn flex-1 p-0.5 text-indigo-600 hover:bg-indigo-50 rounded border border-indigo-200 hover:border-indigo-300 transition-all text-center hover:rotate-180"
                                        data-id="<?php echo htmlspecialchars($data['id_peserta']); ?>" title="Transfer Aset IT">
                                        <i class="fas fa-right-left text-[9px]"></i>
                                    </button>
                                    <button
                                        class="delete-btn flex-1 p-0.5 text-red-600 hover:bg-red-50 rounded border border-red-200 hover:border-red-300 transition-all text-center"
                                        data-id="<?php echo htmlspecialchars($data['id_peserta']); ?>" title="Delete">
                                        <i class="fas fa-trash text-[9px]"></i>
                                    </button>
                            ...
                            <table class="w-full table-sticky">
                                <thead class="bg-gradient-to-r from-gray-800 to-gray-900 text-white">
                                    <tr>
                                        ...
                                        <th class="px-4 py-4 text-left font-semibold sticky-col col-waktu">Waktu</th>
                                        <th class="px-4 py-4 text-left font-semibold sticky-col col-createby">Create By</th>
                                        <th class="px-4 py-4 text-left font-semibold sticky-col col-namabarang">Nama Barang
                                        </th>
                                        <th class="px-4 py-4 text-left font-semibold col-nomoraset">Nomor Aset</th>
                                        <th class="px-4 py-4 text-left font-semibold">Merek</th>
                                        <th class="px-4 py-4 text-left font-semibold">Type</th>
                                ...
                                <tbody>
                                    ...
                                    while ($data = mysqli_fetch_array($hasil)) {
                                        ...
                                        ?>
                                        <tr
                                            class="table-row border-b border-gray-100 <?php echo $rowClass; ?> hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50">
                                            <td class="px-4 py-4 font-medium text-gray-900 sticky-col col-no">
                                                <?php echo $no++; ?></td>
                                            <td class="px-4 py-4 text-gray-700 sticky-col col-waktu">
                                                <?php echo $data["Waktu"]; ?></td>
                                            <td class="px-4 py-4 text-gray-700 sticky-col col-createby">
                                                <?php
                                                // Tampilkan Username - Nama dari akun login yang membuat data
                                                $username = isset($_SESSION['username']) ? $_SESSION['username'] : '-';
                                                $creator_name = isset($_SESSION['Nama_Lengkap']) ? $_SESSION['Nama_Lengkap'] : 'Unknown';
                                                echo htmlspecialchars($username) . ' - ' . htmlspecialchars($creator_name);
                                                ?>
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-gray-700 font-mono text-sm col-nomoraset">
                                                <?php echo htmlspecialchars($data["Nomor_Aset"] ?? ''); ?></td>
                                            <td class="px-4 py-4 text-gray-700"><?php echo htmlspecialchars($data["Merek"]); ?>
                                            </td>
                                            <td class="px-4 py-4 text-gray-700"><?php echo htmlspecialchars($data["Type"]); ?>
                                            </td>
                                            <td class="px-4 py-4 text-gray-700 font-mono text-sm">
                                                <?php echo htmlspecialchars($data["Serial_Number"]); ?></td>
                                            <td class="px-4 py-4 text-gray-700 max-w-xs">
                                            ...
                                            <td class="px-4 py-4 text-gray-700 max-w-xs">
                                                <div class="line-clamp-2 overflow-hidden"
                                                    title="<?php echo htmlspecialchars($data["User_Perangkat"] ?? ''); ?>">
                                                    <?php echo htmlspecialchars($data["User_Perangkat"] ?? ''); ?>
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 text-gray-700 max-w-xs">
                                                <div class="line-clamp-2 overflow-hidden"
                                                    title="<?php echo htmlspecialchars($data["Lokasi"]); ?>">
                                                    <?php echo htmlspecialchars($data["Lokasi"]); ?>
                                                </div>
                                            </td>
                                            <td class="px-4 py-4 text-gray-700">
                                                <?php echo htmlspecialchars($data["Id_Karyawan"]); ?></td>
                                            <td class="px-4 py-4 text-gray-700">
                                                <?php echo htmlspecialchars($data["Jabatan"]); ?></td>

                                            <!-- Modern Status Badges -->
                                            <td class="px-4 py-4">
                                                <span
                                                    class="status-badge <?php echo getStatusBadgeClass($data['Jenis_Barang']); ?>">
                                                    <?php echo htmlspecialchars($data['Jenis_Barang']); ?>
                                                </span>
                                            </td>
                                            <td class="px-4 py-4">
                                                <span
                                                    class="status-badge <?php echo getStatusBadgeClass($data['Status_Barang']); ?>">
                                                    <?php echo htmlspecialchars($data['Status_Barang']); ?>
                                                </span>
                                            </td>
                                            <td class="px-4 py-4">
                                                <span
                                                    class="status-badge <?php echo getStatusBadgeClass($data['Status_LOP']); ?>">
                                                    <?php echo htmlspecialchars($data['Status_LOP']); ?>
                                                </span>
                                            </td>
                                            <td class="px-4 py-4">
                                                <span
                                                    class="status-badge <?php echo getStatusBadgeClass($data['Status_Kelayakan_Barang']); ?>">
                                                    <?php echo htmlspecialchars($data['Status_Kelayakan_Barang']); ?>
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-4 py-4">
                                                <?php if (!empty($data['Photo_Barang'])): ?>
                                                    <div
                                                        class="w-16 h-16 rounded-lg overflow-hidden shadow-md cursor-pointer hover:shadow-lg transition-all duration-300 hover:scale-105">
                                                ...
                                                <?php if (!empty($data['Photo_Depan'])): ?>
                                                    <div
                                                        ...
                                                        <img src="../uploads/<?php echo htmlspecialchars($data['Photo_Depan']); ?>"
                                                            alt="Foto Depan" class="w-full h-full object-cover"
                                                            onclick="openImageModal('../uploads/<?php echo htmlspecialchars($data['Photo_Barang']); ?>')">
                                                    </div>
                                                <?php else: ?>
                                                    <div class="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center">
                                                ...
                                                <?php if (!empty($data['Photo_Belakang'])): ?>
                                                    <div
                                                        ...
                                                        <img src="../uploads/<?php echo htmlspecialchars($data['Photo_Belakang']); ?>"
                                                            alt="Foto Belakang" class="w-full h-full object-cover"
                                                ...
                                                <?php if (!empty($data['Photo_SN'])): ?>
                                                    <div
                                                        ...
                                                        <img src="../uploads/<?php echo htmlspecialchars($data['Photo_SN']); ?>"
                                                            alt="Foto SN" class="w-full h-full object-cover"
                                            ...
                                            <td class="px-4 py-4 sticky-right">
                                                <div class="flex items-center justify-center space-x-2">
                                                    <a href="<?php echo htmlspecialchars(admin_update_url($data['id_peserta']), ENT_QUOTES); ?>"
                                                        class="p-2 text-green-600 hover:bg-green-50 rounded-lg border border-green-200 hover:border-green-300 transition-all duration-300 hover:scale-105">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <button type="button"
                                                        class="transfer-btn p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg border border-indigo-200 hover:border-indigo-300 transition-all duration-300 hover:scale-105 hover:rotate-180"
                                                        data-id="<?php echo htmlspecialchars($data['id_peserta']); ?>"
                                                        title="Transfer Aset IT">
                                                        <i class="fas fa-right-left"></i>
                                                    </button>
                                                    <button
                                                        class="delete-btn p-2 text-red-600 hover:bg-red-50 rounded-lg border border-red-200 hover:border-red-300 transition-all duration-300 hover:scale-105"
                                                        data-id="<?php echo htmlspecialchars($data['id_peserta']); ?>">
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="w-full max-w-4xl flex flex-col items-center space-y-2">
                        ...
                        <nav class="inline-flex rounded-md shadow-sm -space-x-px justify-center">
                            ...
                            if (!empty($brand_filter))
                                $url_params[] = "brand_filter=" . urlencode($brand_filter);
                            if (!empty($status_lop_filter))
                                $url_params[] = "status_lop_filter=" . urlencode($status_lop_filter);
                            if (!empty($status_kelayakan_filter))
                                $url_params[] = "status_kelayakan_filter=" . urlencode($status_kelayakan_filter);
                            if (!empty($start_date))
                                $url_params[] = "start_date=" . urlencode($start_date);
                            if (!empty($end_date))
                                $url_params[] = "end_date=" . urlencode($end_date);
                            if (!empty($view_mode))
                                $url_params[] = "view_mode=" . urlencode($view_mode);
                            $url_suffix = !empty($url_params) ? '&' . implode('&', $url_params) : '';

                            // Previous
                            if ($page > 1): ?>
                                <a href="index.php?page=<?php echo ($page - 1) . $url_suffix; ?>"
                            ...
                            if ($start > 1): ?>
                                <a href="index.php?page=1<?php echo $url_suffix; ?>"
                                    class="pagination-link relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">1</a>
                                <?php if ($start > 2): ?>
                                    <span
                                        class="relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500">...</span>
                                <?php endif; ?>
                            <?php endif; ?>
                            ...
                            <?php for ($i = $start; $i <= $end; $i++): ?>
                                <?php if ($i == $page): ?>
                                    <span
                                        class="relative z-10 inline-flex items-center px-3 py-2 border border-orange-500 bg-orange-50 text-sm font-medium text-orange-600"><?php echo $i; ?></span>
                                <?php else: ?>
                                    <a href="index.php?page=<?php echo $i . $url_suffix; ?>"
                                        class="pagination-link relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50"><?php echo $i; ?></a>
                                <?php endif; ?>
                            <?php endfor; ?>
                            ...
                            <?php if ($end < $total_pages): ?>
                                <?php if ($end < $total_pages - 1): ?>
                                    <span
                                        class="relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500">...</span>
                                <?php endif; ?>
                                <a href="index.php?page=<?php echo $total_pages . $url_suffix; ?>"
                                    class="pagination-link relative inline-flex items-center px-3 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50"><?php echo $total_pages; ?></a>
                            <?php endif; ?>
            ...
            <div class="text-center text-gray-700 mt-6 mb-16">
                <div class="bg-white/80 backdrop-blur-sm rounded-lg p-4 inline-block shadow-lg border border-gray-200">
                    ...
                    <span
                        class="text-xl font-bold bg-gradient-to-r from-blue-500 to-purple-600 bg-clip-text text-transparent"><?php echo $total_records; ?></span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content relative z-10 w-full h-full flex flex-col">
            ...
            <div class="flex items-center justify-between p-4 bg-black/20 backdrop-blur-sm">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <button onclick="zoomIn()"
                        class="p-2 text-white hover:bg-white/20 rounded-lg transition-all duration-200" title="Zoom In">
                        <i class="fas fa-search-plus"></i>
                    </button>

                    <button onclick="resetZoom()"
                        class="px-3 py-2 text-sm text-white hover:bg-white/20 rounded-lg transition-all duration-200"
                        title="Reset View">
                        Reset
                    </button>
                    ...
                    <button onclick="closeImageModal()"
                        class="p-2 text-white hover:bg-white/20 rounded-lg transition-all duration-200" title="Close">
            ...
            <div class="flex-1 flex items-center justify-center overflow-hidden cursor-grab" id="imageContainer">
                <img id="modalImage" src="" alt="Asset Image"
                    class="max-w-none max-h-none object-contain transition-transform duration-200 select-none"
    ...
    <!-- Transfer Aset IT Modal -->
    <div id="transferModal" class="fixed inset-0 z-[9998] opacity-0 pointer-events-none transition-opacity duration-200"
        aria-hidden="true">
        <div id="transferBackdrop"
            class="absolute inset-0 bg-black/40 backdrop-blur-sm opacity-0 transition-opacity duration-200"
            data-transfer-close="1"></div>

        <div class="relative w-full h-full flex items-center justify-center p-4 sm:p-6">
            <div id="transferPanel"
                class="w-full max-h-[90vh] sm:h-auto sm:max-w-2xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col transform transition-all duration-200 scale-95 opacity-0">
                <div
                    class="px-5 py-4 shrink-0 bg-gradient-to-r from-gray-900 to-gray-800 text-white flex items-center justify-between rounded-t-2xl">
                    <div>
                        <h3 class="text-base sm:text-lg font-semibold flex items-center gap-2">
                            <i class="fas fa-right-left"></i>
                            <span>Transfer Aset IT</span>
                        </h3>
                        <p class="text-xs sm:text-sm text-white/80" id="transferAssetSummary">Memuat data...</p>
                    </div>
                    <button type="button" class="p-2 rounded-lg hover:bg-white/10 transition" data-transfer-close="1"
                        aria-label="Tutup">
                        <i class="fas fa-times"></i>
                    </button>
                </div>

                <form id="transferForm" class="p-5 flex-1 overflow-y-auto">
                    <input type="hidden" name="action" value="transfer">
                    <input type="hidden" name="id_peserta" id="transferId" value="">

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-semibold text-gray-800 dark:text-gray-100 mb-1">Mode Aset
                                <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <i class="fas fa-list absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                <select name="action_type" id="transferActionType" required
                                    class="w-full rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 pl-10 pr-3 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-medium appearance-none cursor-pointer">
                                    <option value="TRANSFER">Transfer Aset</option>
                                    <option value="RETURN">Return Aset</option>
                                </select>
                                <div
                        ...
                        <div class="sm:col-span-2">
                            <div
                                class="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-inner">
                                <div>
                                    <div
                                        class="text-[11px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">
                                        Nama Barang</div>
                                    <div class="text-sm font-bold text-gray-900 dark:text-gray-100"
                                        id="transferNamaBarang">-</div>
                                </div>
                                <div>
                                    <div
                                        class="text-[11px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">
                                        Nomor Aset</div>
                                    <div class="text-sm font-mono font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded inline-block"
                                        id="transferNomorAset">-</div>
                                </div>
                                <div>
                                    <div
                                        class="text-[11px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">
                                    ...
                                    <div class="text-sm font-mono text-gray-700 dark:text-gray-300"
                                        id="transferSerialNumber">-</div>
                                </div>
                            </div>
                        </div>

                        <!-- Data Spesifik Transfer / Fields Karyawan -->
                        <div id="transferUsersData"
                            class="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 transition-all duration-300">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">User
                                    Perangkat <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fas fa-user transfer-field-icon absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="nama" id="transferNama" required
                                        class="transfer-select w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        <option value=""></option>
                                        <?php foreach ($transfer_user_perangkat_options as $opt): ?>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Jabatan
                                    <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fas fa-briefcase transfer-field-icon absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="jabatan" id="transferJabatan" required
                                        class="transfer-select w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        <option value=""></option>
                                        <?php foreach ($transfer_jabatan_options as $opt): ?>
                                            <option value="<?php echo htmlspecialchars($opt); ?>">
                                                <?php echo htmlspecialchars($opt); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Employee
                                    ID <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fas fa-id-card transfer-field-icon absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="empleId" id="transferEmpleId" required
                                        class="transfer-select w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        <option value=""></option>
                                        <?php foreach ($transfer_employee_id_options as $opt): ?>
                                            <option value="<?php echo htmlspecialchars($opt); ?>">
                                                <?php echo htmlspecialchars($opt); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Lokasi
                                    <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <i
                                        class="fas fa-map-marker-alt transfer-field-icon absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                    <select name="lokasi" id="transferLokasi" required
                                        class="transfer-select w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        <option value=""></option>
                                        <?php foreach ($transfer_lokasi_options as $opt): ?>
                                            <option value="<?php echo htmlspecialchars($opt); ?>">
                                                <?php echo htmlspecialchars($opt); ?></option>
                                        <?php endforeach; ?>
                                    </select>
                        ...
                        </div> <!-- End of Data Karyawan -->

                        <div id="transferDate1Container" class="sm:col-span-1 transition-all duration-300">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Tgl Serah
                                Terima <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <i
                                    class="fas fa-calendar-check absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                <input type="date" name="tgl_serah_terima" id="transferTglSerah" required
                                    class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            </div>
                        </div>

                        <div class="sm:col-span-1 transition-all duration-300">
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1">Tgl
                                Pengembalian</label>
                            <div class="relative">
                                <i
                                    class="fas fa-calendar-xmark absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                                <input type="date" name="tgl_pengembalian" id="transferTglKembali"
                                    class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            </div>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Catatan
                                Tambahan Khusus Return Aset</label>
                            <div class="relative">
                                <i class="fas fa-note-sticky absolute left-3 top-3 text-gray-400"></i>
                                <textarea name="catatan" id="transferCatatan" rows="3"
                                    class="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 pl-10 pr-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    placeholder="Contoh: Aset dikembalikan normal."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 flex items-center justify-end gap-2">
                        <button type="button" id="transferCancelBtn"
                            ...
                            data-transfer-close="1">Batal</button>
                        <button type="submit" id="transferSubmitBtn"
                            class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2">
    ...
    <script>
        $(document).ready(function () {
            // Logic for Transfer / Return Aset Toggle
            $(document).on('change', '#transferActionType', function () {
                const action = $(this).val();
                const $usersData = $('#transferUsersData');
                const $date1Container = $('#transferDate1Container');
                const reqFields = $('#transferNama, #transferJabatan, #transferEmpleId, #transferLokasi, #transferTglSerah');
                const $tglKembali = $('#transferTglKembali');
                const $submitBtn = $('#transferSubmitBtn');

                if (action === 'RETURN') {
                    $usersData.slideUp(250);
                    $date1Container.slideUp(250);
                    reqFields.prop('required', false);
                    $tglKembali.prop('required', true);
                    $submitBtn.html('<i class="fas fa-undo"></i> Simpan Return');
                } else {
                    $usersData.slideDown(250);
                    $date1Container.slideDown(250);
                    reqFields.prop('required', true);
                    $tglKembali.prop('required', false);
                    $submitBtn.html('<i class="fas fa-floppy-disk"></i> Simpan Transfer');
                }
            });


            // Auto-fill Transfer Asset Modal fields from users table realtime
            $(document).on('select2:select', '#transferNama', function (e) {
                if ($(this).data('autofilling')) return;
                const val = $(this).val();
                if (!val) return;

                $(this).data('autofilling', true);
                $.ajax({
                    url: '../api_get_employee_details.php',
                    type: 'GET',
                    data: { action: 'get_by_name', nama: val },
                    dataType: 'json',
                    success: function (res) {
                        if (res.ok) {
                            if (res.employee_id) autoFillSelect2Transfer('transferEmpleId', res.employee_id);
                            if (res.jabatan) autoFillSelect2Transfer('transferJabatan', res.jabatan);
                            if (res.lokasi) autoFillSelect2Transfer('transferLokasi', res.lokasi);
                        }
                    },
                    complete: () => { $('#transferNama').data('autofilling', false); }
                });
            });

            $(document).on('select2:select', '#transferEmpleId', function (e) {
                if ($(this).data('autofilling')) return;
                const val = $(this).val();
                if (!val) return;
                ...
                $.ajax({
                    url: '../api_get_employee_details.php',
                    type: 'GET',
                    data: { action: 'get_by_id', id: val },
                    dataType: 'json',
                    success: function (res) {
                        if (res.ok) {
                            if (res.nama) autoFillSelect2Transfer('transferNama', res.nama);
                            if (res.jabatan) autoFillSelect2Transfer('transferJabatan', res.jabatan);
                            if (res.lokasi) autoFillSelect2Transfer('transferLokasi', res.lokasi);
                        }
                    },
                    complete: () => { $('#transferEmpleId').data('autofilling', false); }
                });
            });

            function autoFillSelect2Transfer(selectId, value) {
                if (!value) return;
                const $el = $('#' + selectId);
                if ($el.length === 0) return;

                const exists = Array.from($el[0].options || []).some(opt => String(opt.value) === String(value));
                if (!exists) {
            ...
            // TAMBAHAN: Alert Sukses/Error setelah Delete (cek parameter URL)
            ...
            if (urlParams.get('success') === '1') {
                Swal.fire({
                    ...
                    customClass: {
                        popup: 'animate__animated animate__fadeInDown'  // Tambah animasi jika punya Animate.css
                    }
                }).then(() => {
                    // Clear parameter dari URL (agar tidak muncul lagi saat refresh)
                    const newUrl = window.location.pathname;
                    window.history.replaceState({}, document.title, newUrl);
                });
            } else if (urlParams.get('error') === '1') {
                const errorMsg = urlParams.get('message') || 'Terjadi kesalahan saat menghapus data.';
                Swal.fire({
                    icon: 'error',
                    title: 'Gagal!',
                    text: errorMsg,
                    confirmButtonText: 'OK',
                    confirmButtonColor: '#ef4444',  // Merah error
                    customClass: {
                        popup: 'animate__animated animate__fadeInDown'
                    }
                }).then(() => {
                    // Clear parameter dari URL
                    const newUrl = window.location.pathname;
                    window.history.replaceState({}, document.title, newUrl);
                });
            }
            // ✅ FIX: Handle status parameter dari redirect create.php
            ...
            function loadPage(page) {
                ...
                fetch(fetchUrl)
                    ...
                    .then(data => {
                        ...
                        $('.delete-btn').off('click').on('click', deleteButtonHandler);
                    })
                    .catch(error => {
            ...
            // Delete button handler (separate function fordelegation)
            function deleteButtonHandler(e) {
                ...
                Swal.fire({
                    ...
                    customClass: {
                        ...
                        confirmButton: 'hover:scale-105',
                        cancelButton: 'hover:scale-105'
                    }
                }).then((result) => {
            ...
            $(document).on('click', '.delete-btn', deleteButtonHandler);
            ...
            if (typeof $.fn.select2 !== 'undefined') {
                ...
                $('.select2-filter').select2({
                    ...
                    dropdownParent: $('#main-content-wrapper'),  // Hindari z-index issue dengan sidebar/modal
                    language: {
                        noResults: function () {
        ...
        // Buka Modal Edit dan Populate Data
        editBtns.forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                // Opsional: fokus ke field pertama
                setTimeout(() => {
                    document.getElementById('editNama_Lengkap')?.focus();
                }, 300);
            });
        });
        ...
        editForm.addEventListener('submit', function (e) {
            ...
            const requiredFields = ['Nama_Lengkap', 'username', 'Jabatan_Level', 'Divisi', 'role', 'Email', 'Status_Akun'];
            let isValid = true;
        ...
        // Handle Delete via AJAX dengan SweetAlert2 untuk konfirmasi modern
        deleteBtns.forEach(btn => {
            btn.addEventListener('click', function () {
                const id = this.dataset.id;

                // Alert konfirmasi modern dengan animasi
                Swal.fire({
                    ...
                    confirmButtonText: 'Ya, Hapus!',
                    cancelButtonText: 'Batal',
                    animation: true,  // Aktifkan animasi default (fade-in)
                    customClass: {
                        popup: 'animate__animated animate__bounceIn',  // Tambah animasi bounce (butuh Animate.css jika ingin lebih)
                    }
                }).then((result) => {
                    if (result.isConfirmed) {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    Swal.fire({
                                        ...
                                        location.reload(); // Reload setelah alert tutup
                                    });
                                } else {
                                    // Alert error
                                    Swal.fire({
        ...
        (function () {
            ...
            const summaryEl = document.getElementById('transferAssetSummary');
            const namaBarangEl = document.getElementById('transferNamaBarang');
            const nomorAsetEl = document.getElementById('transferNomorAset');
            const serialEl = document.getElementById('transferSerialNumber');
            const idEl = document.getElementById('transferId');

            const namaEl = document.getElementById('transferNama');
            const jabatanEl = document.getElementById('transferJabatan');
            const emplEl = document.getElementById('transferEmpleId');
            const lokasiEl = document.getElementById('transferLokasi');
            const tglSerahEl = document.getElementById('transferTglSerah');
            const tglKembaliEl = document.getElementById('transferTglKembali');
            const catatanEl = document.getElementById('transferCatatan');

            const submitBtn = document.getElementById('transferSubmitBtn');

            let __transferSelect2Inited = false;

            function initTransferSelect2() {
                if (__transferSelect2Inited) return;
                if (!window.jQuery || !jQuery.fn || typeof jQuery.fn.select2 === 'undefined') return;
                const $modal = jQuery('#transferModal');
                jQuery('.transfer-select').each(function () {
                ...
                __transferSelect2Inited = true;
            }

            function setSelectOrInputValue(el, value) {
                if (!el) return;
                const v = (value == null) ? '' : String(value);

                // If it's a select, ensure the option exists (needed for Select2 + tags)
                if (el.tagName && el.tagName.toLowerCase() === 'select') {
                    const trimmed = v.trim();
                    if (trimmed !== '') {
                        const exists = Array.from(el.options || []).some(opt => String(opt.value) === trimmed);
                        if (!exists) {
                            try {
                                el.add(new Option(trimmed, trimmed, true, true));
                            } catch (e) {
                                // ignore
                            }
                        }
            ...
            function setBusy(busy) {
                ...
                if (busy) {
                    ...
                    submitBtn.innerHTML = 'Simpan Transfer';
                }
                form.querySelectorAll('input, textarea, select, button').forEach(el => {
                    if (el === submitBtn) return;
                    if (el.getAttribute('data-transfer-close') === '1') return;
                    el.disabled = !!busy;
                });
            }
            ...
            async function openTransferModal(idPeserta) {
                initTransferSelect2();
                openModal();
                setBusy(true);
                ...
                nomorAsetEl.textContent = '-';
                serialEl.textContent = '-';
                idEl.value = String(idPeserta || '');

                // Reset Mode Aset to TRANSFER
                const actionTypeEl = document.getElementById('transferActionType');
                if (actionTypeEl) {
                    actionTypeEl.value = 'TRANSFER';
                    if (window.jQuery) jQuery(actionTypeEl).trigger('change');
                }

                // Default input
                setSelectOrInputValue(namaEl, '');
                setSelectOrInputValue(jabatanEl, '');
                setSelectOrInputValue(emplEl, '');
                setSelectOrInputValue(lokasiEl, '');
                tglSerahEl.value = todayISO();
                tglKembaliEl.value = '';
                catatanEl.value = '';
                ...
                try {
                    ...
                    const resp = await fetch('transfer_asset.php?action=get&id_peserta=' + encodeURIComponent(String(idPeserta)), {
                    ...
                    if (!resp.ok || !data || data.ok !== true) {
                        throw new Error((data && data.error) ? data.error : 'Gagal mengambil data');
                    }

                    const asset = data.asset || {};
                    const riwayat = Array.isArray(data.riwayat) ? data.riwayat : [];
                    const last = riwayat.length ? riwayat[riwayat.length - 1] : null;

                    namaBarangEl.textContent = asset.Nama_Barang || '-';
                    nomorAsetEl.textContent = asset.Nomor_Aset || '-';
                    serialEl.textContent = asset.Serial_Number || '-';
                    summaryEl.textContent = (asset.Nama_Barang || '-') + ' • ' + (asset.Nomor_Aset || '-') + ' • ' + (asset.Serial_Number || '-');

                    // Autofill prefer dari riwayat terakhir, fallback dari kolom aktif
                    setSelectOrInputValue(namaEl, (last && last.nama) ? last.nama : (asset.User_Perangkat || ''));
                    setSelectOrInputValue(jabatanEl, (last && last.jabatan) ? last.jabatan : (asset.Jabatan || ''));
                    setSelectOrInputValue(emplEl, (last && (last.empleId || last.emplId)) ? (last.empleId || last.emplId) : (asset.Id_Karyawan || ''));
                    setSelectOrInputValue(lokasiEl, (last && last.lokasi) ? last.lokasi : (asset.Lokasi || ''));

                    if (last && last.tgl_serah_terima) {
                        tglSerahEl.value = String(last.tgl_serah_terima);
                    }
                    if (!tglSerahEl.value) tglSerahEl.value = todayISO();
                } catch (err) {
                    console.error(err);
                    const msg = (err && err.name === 'AbortError') ? 'Request timeout (15 detik). Coba lagi.' : (err && err.message ? err.message : 'Gagal memuat data transfer');
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
                        text: msg,
                ...
                setTimeout(() => {
                    try {
                        if (window.jQuery && jQuery.fn && jQuery.fn.select2 && namaEl && namaEl.tagName.toLowerCase() === 'select') {
                            const $sel = jQuery(namaEl);
                            const $container = $sel.next('.select2');
                            const $focusTarget = $container.find('.select2-selection');
            ...
            // Expose global for inline usage if needed
            window.openTransferModal = openTransferModal;

            // Delegated click for dynamic (AJAX) content
            document.addEventListener('click', function (e) {
            ...
            modal.addEventListener('click', function (e) {
                const closeEl = e.target.closest('[data-transfer-close="1"]');
                if (!closeEl) return;
                e.preventDefault();
                closeModal();
            });
            ...
            form.addEventListener('submit', async function (e) {
                e.preventDefault();
                // IMPORTANT: Jangan disable dulu sebelum bikin FormData.
                // Disabled form controls tidak ikut terkirim -> action/id_peserta bisa hilang.
                const fd = new FormData(form);
                setBusy(true);
                try {
                    const controller = new AbortController();
                    const timeoutId = setTimeout(() => controller.abort(), 20000);
                    const resp = await fetch('transfer_asset.php', {
                        method: 'POST',
                        body: fd,
                        credentials: 'same-origin',
                        signal: controller.signal
                    ...
                    const data = await resp.json();
                    if (!resp.ok || !data || data.ok !== true) {
                        throw new Error((data && data.error) ? data.error : 'Gagal menyimpan transfer');
                    }
                    await Swal.fire({
                        icon: 'success',
                        title: 'Berhasil',
                        text: 'Transfer aset berhasil disimpan ke Riwayat Barang.',
                        timer: 1800,
                        timerProgressBar: true,
                        showConfirmButton: false
                    });
                    ...
                    console.error(err);
                    const msg = (err && err.name === 'AbortError') ? 'Request timeout (20 detik). Coba lagi.' : (err && err.message ? err.message : 'Terjadi kesalahan saat menyimpan');
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
                        text: msg,
    ...
    <script>
        // Bootstrap fallback: pastikan fitur Transfer tetap jalan walaupun ada JS error lain sebelumnya
        (function () {
            if (window.__transferBootstrapFallbackInitialized) return;
            window.__transferBootstrapFallbackInitialized = true;

            function bindClickHandler() {
                if (window.__transferClickBound) return;
                window.__transferClickBound = true;

                // jQuery delegated click (lebih tahan terhadap konten AJAX)
                if (window.jQuery) {
                    jQuery(document).off('click.transferAsset').on('click.transferAsset', '.transfer-btn', function (e) {
                        e.preventDefault();
                        ...
                        if (typeof window.openTransferModal === 'function') {
                            ...
                            console.error('openTransferModal belum tersedia');
                        }
                    });
                } else {
                    document.addEventListener('click', function (e) {
                        const btn = e.target && e.target.closest ? e.target.closest('.transfer-btn') : null;
                        if (!btn) return;
                        e.preventDefault();
                        const id = btn.getAttribute('data-id') || '';
                        if (!id) return;
            ...
            // Kalau fungsi sudah ada, cukup pastikan click handler kebind
            if (typeof window.openTransferModal === 'function') {
                bindClickHandler();
                return;
            }

            // Definisikan openTransferModal bila belum ada
            const modal = document.getElementById('transferModal');
            if (!modal) {
                console.error('transferModal tidak ditemukan di DOM');
                return;
            }
            ...
            function setSelectOrInputValue(el, value) {
                ...
                if (el.tagName && el.tagName.toLowerCase() === 'select') {
                    ...
                    if (trimmed !== '') {
                        const exists = Array.from(el.options || []).some(opt => String(opt.value) === trimmed);
                        if (!exists) {
                            try { el.add(new Option(trimmed, trimmed, true, true)); } catch (e) { }
                        }
                        el.value = trimmed;
                    } else {
                        el.value = '';
            ...
            function setBusy(busy) {
                if (submitBtn) submitBtn.disabled = !!busy;
                if (submitBtn) submitBtn.innerHTML = busy ? 'Menyimpan...' : 'Simpan Transfer';
                if (!form) return;
                form.querySelectorAll('input, textarea, select, button').forEach(el => {
                    if (el === submitBtn) return;
                    if (el.getAttribute('data-transfer-close') === '1') return;
            ...
            window.openTransferModal = async function (idPeserta) {
                initTransferSelect2();
                openModal();
                setBusy(true);

                if (summaryEl) summaryEl.textContent = 'Memuat data...';
                if (namaBarangEl) namaBarangEl.textContent = '-';
                if (nomorAsetEl) nomorAsetEl.textContent = '-';
                if (serialEl) serialEl.textContent = '-';
                if (idEl) idEl.value = String(idPeserta || '');
                ...
                setSelectOrInputValue(emplEl, '');
                setSelectOrInputValue(lokasiEl, '');
                if (tglSerahEl) tglSerahEl.value = todayISO();
                ...
                try {
                    const controller = new AbortController();
                    const timeoutId = setTimeout(() => controller.abort(), 15000);
                    const resp = await fetch('transfer_asset.php?action=get&id_peserta=' + encodeURIComponent(String(idPeserta)), { credentials: 'same-origin', signal: controller.signal });
                    clearTimeout(timeoutId);
                    const data = await resp.json();
                    if (!resp.ok || !data || data.ok !== true) {
                        throw new Error((data && data.error) ? data.error : 'Gagal mengambil data');
                    ...
                    if (namaBarangEl) namaBarangEl.textContent = asset.Nama_Barang || '-';
                    if (nomorAsetEl) nomorAsetEl.textContent = asset.Nomor_Aset || '-';
                    if (serialEl) serialEl.textContent = asset.Serial_Number || '-';
                    if (summaryEl) summaryEl.textContent = (asset.Nama_Barang || '-') + ' • ' + (asset.Nomor_Aset || '-') + ' • ' + (asset.Serial_Number || '-');
                    ...
                    if (last && last.tgl_serah_terima && tglSerahEl) {
                        tglSerahEl.value = String(last.tgl_serah_terima);
                    }
                    if (tglSerahEl && !tglSerahEl.value) tglSerahEl.value = todayISO();
                } catch (err) {
                    console.error(err);
                    const msg = (err && err.name === 'AbortError') ? 'Request timeout (15 detik). Coba lagi.' : (err && err.message ? err.message : 'Gagal memuat data transfer');
                    if (window.Swal) {
            ...
            // close handlers
            modal.addEventListener('click', function (e) {
                const closeEl = e.target && e.target.closest ? e.target.closest('[data-transfer-close="1"]') : null;
                if (!closeEl) return;
                e.preventDefault();
                closeModal();
            });
            ...
            if (form) {
                form.addEventListener('submit', async function (e) {
                    ...
                    try {
                        const controller = new AbortController();
                        const timeoutId = setTimeout(() => controller.abort(), 20000);
                        const resp = await fetch('transfer_asset.php', { method: 'POST', body: fd, credentials: 'same-origin', signal: controller.signal });
                        clearTimeout(timeoutId);
                        const data = await resp.json();
                        if (!resp.ok || !data || data.ok !== true) {
                            throw new Error((data && data.error) ? data.error : 'Gagal menyimpan transfer');
