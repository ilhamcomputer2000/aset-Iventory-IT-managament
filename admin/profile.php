...
if (isset($_POST['action']) && $_POST['action'] === 'update_info') {
    ...
    $allowedFields = ['Nama_Lengkap', 'Email', 'No_Telp_WA', 'Divisi', 'Jabatan_Level', 'Region'];
    if (!in_array($field, $allowedFields)) {
        echo json_encode(['success' => false, 'message' => 'Field tidak valid']);
        exit;
    }

    $stmt = $kon->prepare("UPDATE users SET $field = ? WHERE id = ?");
    $stmt->bind_param('si', $value, $userId);
    if ($stmt->execute()) {
        // Update session if relevant
        if ($field === 'Nama_Lengkap')
            $_SESSION['Nama_Lengkap'] = $value;
        if ($field === 'Jabatan_Level')
            $_SESSION['Jabatan_Level'] = $value;
...
$initial = strtoupper(substr($user['Nama_Lengkap'] ?? 'A', 0, 1));
...
<body class="bg-gray-50">
        ...
        <main class="mt-16 min-h-screen p-4 sm:p-6 pb-24">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-gray-500 text-sm mt-0.5">Kelola informasi profil dan foto Anda</p>
                </div>
                    ...
                    <div class="px-4 sm:px-8 pb-6 relative z-10">
                        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                            ...
                            <div class="avatar-wrapper relative flex-shrink-0 w-[120px] h-[120px] -mt-12 sm:-mt-16">
                                <?php if ($profilePic): ?>
                                    <img src="<?php echo htmlspecialchars($profilePic); ?>" alt="Profile"
                                        class="profile-avatar w-full h-full" id="profile-img">
                                <?php else: ?>
                                    <div class="profile-avatar-placeholder w-full h-full" id="profile-img-placeholder">
                                        <?php echo $initial; ?>
                                    </div>
                                <?php endif; ?>
                            ...
                            <div class="text-center sm:text-left flex-1 min-w-0 sm:mt-4">
                                <h2 class="text-xl sm:text-2xl font-bold text-gray-900 break-words leading-tight">
                                    <?php echo htmlspecialchars($user['Nama_Lengkap']); ?>
                                </h2>
                                <p class="text-sm text-gray-500 mt-1 break-words">
                                    <?php echo htmlspecialchars($user['Jabatan_Level']); ?> &mdash;
                                    <?php echo htmlspecialchars($user['Divisi']); ?>
                                </p>
                                <div class="flex items-center gap-2 mt-3 justify-center sm:justify-start flex-wrap">
                                    <span class="role-badge"><i
                                            class="fas fa-shield-alt mr-1"></i><?php echo htmlspecialchars(ucfirst(str_replace('_', ' ', $user['role']))); ?></span>
                                    <span
                                        class="status-badge <?php echo strtolower($user['Status_Akun']) === 'aktif' ? 'status-aktif' : 'status-nonaktif'; ?>">
                                        <i
                                            class="fas fa-circle text-[6px] mr-1"></i><?php echo htmlspecialchars($user['Status_Akun']); ?>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-center flex-wrap gap-2 pt-2 sm:pt-0 sm:mt-5">
                                <button onclick="document.getElementById('photo-input').click()"
                                    class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold rounded-lg transition flex items-center gap-1.5 shadow-sm">
                                    <i class="fas fa-camera"></i> Ganti Foto
                                </button>
                                <?php if ($profilePic): ?>
                                    <button onclick="deletePhoto()"
                                        class="px-3 py-2 border border-gray-200 bg-white text-gray-500 hover:text-red-500 hover:border-red-200 text-xs font-semibold rounded-lg transition shadow-sm"
                                        id="btn-delete-photo">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-row" data-field="Nama_Lengkap">
                            <div class="info-label"><i class="fas fa-id-badge mr-2 text-gray-400"></i>Nama Lengkap</div>
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['Nama_Lengkap']); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'Nama_Lengkap')"><i
                        ...
                        <div class="info-row">
                            ...
                            <div class="info-value">
                                <span
                                    class="font-mono text-gray-600"><?php echo htmlspecialchars($user['username']); ?></span>
                            </div>
                        </div>
                        ...
                        <div class="info-row" data-field="Email">
                            ...
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['Email'] ?: '—'); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'Email')"><i
                                        class="fas fa-pen"></i></button>
                            </div>
                        </div>

                        <div class="info-row" data-field="No_Telp_WA">
                            <div class="info-label"><i class="fab fa-whatsapp mr-2 text-gray-400"></i>No. Telp / WA
                            </div>
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['No_Telp_WA'] ?: '—'); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'No_Telp_WA')"><i
                                        class="fas fa-pen"></i></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="info-row" data-field="Divisi">
                            <div class="info-label"><i class="fas fa-building mr-2 text-gray-400"></i>Divisi</div>
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['Divisi'] ?: '—'); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'Divisi')"><i
                                        class="fas fa-pen"></i></button>
                            </div>
                        </div>

                        <div class="info-row" data-field="Jabatan_Level">
                            <div class="info-label"><i class="fas fa-user-tie mr-2 text-gray-400"></i>Jabatan</div>
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['Jabatan_Level'] ?: '—'); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'Jabatan_Level')"><i
                                        class="fas fa-pen"></i></button>
                            </div>
                        </div>

                        <div class="info-row" data-field="Region">
                            <div class="info-label"><i class="fas fa-map-marker-alt mr-2 text-gray-400"></i>Region</div>
                            <div class="info-value">
                                <span
                                    class="field-display"><?php echo htmlspecialchars($user['Region'] ?: '—'); ?></span>
                                <button class="edit-btn" onclick="editField(this, 'Region')"><i
                                        class="fas fa-pen"></i></button>
                            </div>
                        </div>
                        ...
                        <div class="info-row">
                            ...
                            <div class="info-value">
                                <span
                                    class="role-badge"><?php echo htmlspecialchars(ucfirst(str_replace('_', ' ', $user['role']))); ?></span>
                            </div>
                        </div>

                        <div class="info-row">
                            <div class="info-label"><i class="fas fa-toggle-on mr-2 text-gray-400"></i>Status Akun</div>
                            <div class="info-value">
                                <span
                                    class="status-badge <?php echo strtolower($user['Status_Akun']) === 'aktif' ? 'status-aktif' : 'status-nonaktif'; ?>">
                        ...
                        <div class="info-row">
                            <div class="info-label"><i class="fas fa-calendar-alt mr-2 text-gray-400"></i>Terdaftar
                            </div>
    ...
    <script>
        ...
        function editField(btn, fieldName) {
            ...
            const displayEl = valueEl.querySelector('.field-display');
            const currentVal = displayEl.textContent.trim();
            const actualVal = currentVal === '—' ? '' : currentVal;
            ...
            inputWrap.className = 'flex items-center gap-2 flex-1';
            inputWrap.innerHTML = `
        <input type="text" value="${actualVal.replace(/"/g, '&quot;')}" class="flex-1" autofocus>
        <button class="save-btn" onclick="saveField(this, '${fieldName}')"><i class="fas fa-check"></i></button>
        <button class="cancel-btn" onclick="cancelEdit(this)"><i class="fas fa-times"></i></button>
    `;
            valueEl.appendChild(inputWrap);
            inputWrap.querySelector('input').focus();

            // Enter to save
            inputWrap.querySelector('input').addEventListener('keydown', function (e) {
                if (e.key === 'Enter') inputWrap.querySelector('.save-btn').click();
                if (e.key === 'Escape') inputWrap.querySelector('.cancel-btn').click();
            });
        }

        function saveField(btn, fieldName) {
            const inputWrap = btn.parentElement;
            const row = btn.closest('.info-row');
            const valueEl = row.querySelector('.info-value');
            const displayEl = valueEl.querySelector('.field-display');
            const editBtn = valueEl.querySelector('.edit-btn');
            const newVal = inputWrap.querySelector('input').value.trim();
