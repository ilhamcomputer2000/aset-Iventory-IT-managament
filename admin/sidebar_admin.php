...
<aside id="sidebar" class="fixed top-0 left-0 h-screen w-60 bg-white border-r border-gray-200 z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-lg lg:shadow-none overflow-y-auto">
    ...
    <div class="p-4 pt-5 border-b border-gray-200">
        <div class="flex items-center space-x-3">
            <div class="w-10 h-10 bg-gradient-to-br from-orange-400 to-orange-600 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0">
    ...
    <div class="px-4 py-3 border-b border-gray-200">
        <div class="flex items-center space-x-3">
            ...
            <div class="min-w-0">
                <span class="text-sm font-semibold text-gray-900 block truncate"><?php echo htmlspecialchars($Nama_Lengkap); ?></span>
                <span class="text-[11px] text-gray-500 truncate block"><?php echo htmlspecialchars($Jabatan_Level); ?></span>
            </div>
        </div>
    </div>
    ...
    <nav class="mt-2 px-3 pb-20">
        ...
        <a href="<?php echo htmlspecialchars($_adminLinks['dashboard']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isDashboard ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-tachometer-alt text-base w-5 text-center <?php echo $isDashboard ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isDashboard ? 'font-semibold' : 'font-medium'; ?>">Dashboard</span>
        </a>

        <a href="<?php echo htmlspecialchars($_adminLinks['assets']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isAssets ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-cogs text-base w-5 text-center <?php echo $isAssets ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isAssets ? 'font-semibold' : 'font-medium'; ?>">Assets IT</span>
        </a>

        <a href="<?php echo htmlspecialchars($_adminLinks['lacak']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isLacak ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-search-location text-base w-5 text-center <?php echo $isLacak ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isLacak ? 'font-semibold' : 'font-medium'; ?>">Lacak Asset</span>
        </a>

        <a href="<?php echo htmlspecialchars($_adminLinks['serah_terima']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isSerahTerima ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-file-alt text-base w-5 text-center <?php echo $isSerahTerima ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isSerahTerima ? 'font-semibold' : 'font-medium'; ?>">Form Serah Terima</span>
        </a>

        <a href="<?php echo htmlspecialchars($_adminLinks['ticket']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isTicket ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-ticket-alt text-base w-5 text-center <?php echo $isTicket ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isTicket ? 'font-semibold' : 'font-medium'; ?>">Ticket</span>
        </a>

        <a href="<?php echo htmlspecialchars($_adminLinks['request_pinjaman']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 <?php echo $isRequestPinjaman ? 'bg-orange-50 text-orange-700 border-l-[3px] border-orange-500' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'; ?>">
            <i class="fas fa-hand-holding text-base w-5 text-center <?php echo $isRequestPinjaman ? 'text-orange-500' : 'text-gray-400'; ?>"></i>
            <span class="text-sm <?php echo $isRequestPinjaman ? 'font-semibold' : 'font-medium'; ?>">Request Pinjaman</span>
        </a>
        ...
        <a href="<?php echo htmlspecialchars($_adminLinks['logout']); ?>" class="flex items-center space-x-3 py-2.5 px-3 rounded-lg mb-1 transition-all duration-200 text-red-500 hover:bg-red-50 hover:text-red-600">
    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-14">
            <div class="flex items-center space-x-4">
                <button id="hamburger-btn" class="p-3 rounded-lg bg-white/20 backdrop-blur-sm text-white shadow-md hover:bg-white/30 transition-all duration-300 hover:scale-105 items-center justify-center min-w-[44px] min-h-[44px]">
                    <i id="hamburger-icon" class="fas fa-bars text-lg"></i>
                </button>
            </div>
            <div class="flex items-center space-x-3">
                <!-- Notification Bell -->
                <button id="notif-bell-btn" class="relative p-3 rounded-lg bg-white/20 backdrop-blur-sm text-white shadow-md hover:bg-white/30 transition-all duration-300 hover:scale-105 items-center justify-center min-w-[44px] min-h-[44px]" title="Notifikasi">
                    <i class="fas fa-bell text-lg"></i>
                    <span id="notif-badge" class="hidden absolute -top-1 -right-1 min-w-[20px] h-5 px-1 bg-red-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center ring-2 ring-orange-500 animate-pulse">0</span>
                </button>
            </div>
        </div>
    </div>
...
<div id="notif-overlay" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[60] transition-opacity duration-300 opacity-0 pointer-events-none"></div>

<!-- Notification Slide Panel -->
<div id="notif-panel" class="fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white/95 backdrop-blur-xl z-[61] shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col">
    <!-- Panel Header -->
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-500 to-orange-600">
        <div class="flex items-center space-x-3">
        ...
        <div class="flex items-center space-x-2">
            <button id="notif-mark-all" class="px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-medium rounded-lg transition-all duration-200" title="Tandai semua dibaca">
                <i class="fas fa-check-double mr-1"></i> Tandai Dibaca
            </button>
            <button id="notif-close" class="p-2 text-white/80 hover:text-white hover:bg-white/20 rounded-lg transition-all duration-200">
...
(function() {
    ...
    function getNavUrl(type) {
        const map = {
            'ticket_created': '<?php echo app_abs_path("admin/ticket"); ?>',
            'ticket_closed': '<?php echo app_abs_path("admin/ticket"); ?>',
            'ticket_status_changed': '<?php echo app_abs_path("admin/ticket"); ?>',
            'ticket_approval_needed': '<?php echo app_abs_path("admin/ticket"); ?>',
            'pinjaman_request': '<?php echo app_abs_path("admin/request_pinjaman"); ?>',
            'pinjaman_approved': '<?php echo app_abs_path("admin/request_pinjaman"); ?>',
            'pinjaman_rejected': '<?php echo app_abs_path("admin/request_pinjaman"); ?>'
        };
