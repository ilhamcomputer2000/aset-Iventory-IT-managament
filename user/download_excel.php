<?php
// Download daftar asset IT (user) as XLSX.
...
function export_decode_riwayat_simple($raw): string {
...
function export_attach_image_if_exists($sheet, string $absPath, string $coord, string $name): void
{
    ...
    try {
        ...
        // Skip gambar jika ada error di hosting (file corrupt/permission/unsupported)
        return;
    }
}
...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sheet->setCellValue('J' . $rowNum, export_decode_riwayat_simple($row['Riwayat_Barang'] ?? ''));
