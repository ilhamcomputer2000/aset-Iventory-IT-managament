...
if ($__debug) {
    ...
    header('X-Debug-Update-Build: 2026-03-07');
} else {
    ini_set('display_errors', '0');
}
ini_set('log_errors', '1');
// Log to project temp folder if possible (../temp from /admin)
...
if ($__debug) {
    error_log('UPDATE.PHP build=2026-03-07 method=' . ($_SERVER['REQUEST_METHOD'] ?? '') . ' id_peserta=' . (string)($_GET['id_peserta'] ?? $_POST['id_peserta'] ?? ''));
}
...
// Cek apakah ada parameter id_peserta di URL atau POST
...
// Ambil id_peserta dari GET atau POST
    ...
    <div class="container mx-auto px-4">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (!isset($swal_icon)) {
                ...
                $buildUpdateSql = function(bool $includeNomorAset, bool $includeHargaBarang, bool $includeTahunRilis, bool $includeWaktuPembelian, bool $includeNamaVendor, bool $includeKategoriPembelian, bool $includeLinkPembelian) use (
                    ...
                    if ($includeTahunRilis) {
                        $set .= "Tahun_Rilis = '$Tahun_Rilis_sql',\n";
                    }
                ...
                $includeHargaBarang = $hasCol('Harga_Barang');
                $includeTahunRilis = $hasCol('Tahun_Rilis');
                $includeWaktuPembelian = $hasCol('Waktu_Pembelian');
                $includeNamaVendor = $hasCol('Nama_Toko_Pembelian');
                $includeKategoriPembelian = $hasCol('Kategori_Pembelian');
                $includeLinkPembelian = $hasCol('Link_Pembelian');
                ...
                if (!$includeTahunRilis && trim((string)$Tahun_Rilis) !== '') {
            ...
            if (isset($swal_icon) && $swal_icon) {
                $returnUrl = $_SERVER['PHP_SELF'] . '?id_peserta=' . rawurlencode((string)$id_peserta);
                if ($__debug) {
                    $returnUrl .= '&debug=1';
                }
                if (ob_get_length()) {
